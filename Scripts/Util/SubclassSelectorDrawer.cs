...
namespace Util
{
    ...
    public class SubclassSelectorDrawer : PropertyDrawer
    {
        ...
        private void GetAllInheritedTypes(Type baseType, bool includeMono)
        {
            ...
            inheritedTypes = AppDomain.CurrentDomain
                .GetAssemblies()
        ...
        {
            typePopupNameArray = inheritedTypes
                .Select(type => type == null ? "<null>" : type.ToString())
                .ToArray();
            typeFullNameArray = inheritedTypes
                .Select(
                    type =>
                        type == null
                            ? ""
                            : string.Format(
                                "{0} {1}",
                                type.Assembly.ToString().Split(',')[0],
                                type.FullName
                            )
                )
                .ToArray();
        ...
        private void UpdatePropertyToSelectedTypeIndex(
